...
class Bluetooth private constructor(private val context: Context) {
    ...
    private fun teardownConnection(device: BluetoothDevice) {
        if (device.isConnected()) {
            Log.d("BLE","Disconnecting from ${device.address}")
            ...
            Log.d("BLE","Not connected to ${device.address}, cannot teardown connection!")
        }
    }
    ...
    private fun scanCallbackFactory(observer: MutableSharedFlow<ScanResult>) =
        object : ScanCallbackWrapper(observer) {
            ...
            override fun onScanResult(callbackType: Int, result: ScanResult?) {
                Log.d("BLE","Received scan result: ${result?.device?.name ?: ""}")
                result?.let {
                    runBlocking {
                        flow.emit(it)
                    }
    ...
    private val connectedCallback = object : BluetoothGattCallback() {
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    Log.d("BLE","onConnectionStateChange: connected to $deviceAddress")
                    BleJob.Connect.resumeWithSuccess(gatt)
                    // Notify generic listeners
                    genericListeners.forEach { it.get()?.onConnect?.invoke(gatt.device) }
                } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                    Log.d("BLE","onConnectionStateChange: disconnected from $deviceAddress")
                    teardownConnection(gatt.device)
                } else {
                    TODO("Not handled")
                }
            } else {
                Log.d("BLE","onConnectionStateChange: status $status encountered for $deviceAddress!")
                teardownConnection(gatt.device)
            }
        }
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
            with(gatt) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    Log.d("BLE","Discovered ${services.size} services for ${gatt.device.address}")
                    printGattTable()
        ...
        override fun onCharacteristicRead(
            ...
            when (status) {
                BluetoothGatt.GATT_SUCCESS -> {
                    Log.d("BLE","Read characteristic ${characteristic.uuid} : value: ${value.toHexString()}")
        ...
        override fun onCharacteristicWrite(
            ...
            when (status) {
                BluetoothGatt.GATT_SUCCESS -> {
                    Log.d("BLE","Wrote characteristic ${characteristic.uuid}")
        ...
        override fun onDescriptorWrite(
            ...
            when (status) {
                BluetoothGatt.GATT_SUCCESS -> {
                    Log.d("BLE","Wrote to descriptor ${descriptor.uuid}")
        ...
        override fun onCharacteristicChanged(
            ...
            Log.d("BLE","Characteristic ${characteristic.uuid} changed | value: ${value.toHexString()}")
    ...
    suspend fun enableNotification(
        ...
        Log.d("BLE","Enabling notifications for $characteristic")
        val gatt = gattFromAddress(deviceAddress)
            ?: return Result.failure(Exception("No device found with address $deviceAddress"))
    ...
    suspend fun writeCharacteristic(
        ...
        return BleJob.Write.suspendWithTimeout(5000) {
            Log.d("BLE","Writing characteristic $characteristic ==> ${payload.toHexString()}")
